...
class DistanceMapBuilder:
    ...
    def set_atom(self, atom):
        if atom.casefold() not in ['ca', 'cb']: #.casefold()返回小写字符串
    ...
    def generate_map_for_pdb(self, structure_container):
        ...
        for chain_name in structure_container.chains:
            ...
            if chain['seqres-seq'] is not None and len(chain['seqres-seq']) > 0:#该链有残基
                ...
                for i in range(len(aligned_atom_seq)):
                    ...
                    if current_aligned_atom_residue_letter != '-' and mask[i] == '|':
                        #print(x for x in reindexed_residues[picked_residues:picked_residues + 5])
                ...
                final_seq_three_letter_codes = ''.join([r.resname if r is not None else 'XXX' for r in final_residue_list])
                final_seq_one_letter_codes = seq1(final_seq_three_letter_codes, undef_code='-', custom_map=protein_letters_3to1)
                ...
                if self.pedantic and len(final_residue_list) != len(seqres_seq):
                    raise Exception(f"Somehow the final residue list {len(final_residue_list)} doesn't match the size of the SEQRES seq {len(seqres_seq)}")
                ...
                cmap = self.__residue_list_to_contact_map(final_residue_list, len(seqres_seq))
                contact_maps.with_map_for_chain(chain_name, self.__residue_list_to_contact_map(final_residue_list, len(seqres_seq)))
            else:
    ...
    """def generate_map_for_pdb(self, structure_container):
        ...
        contact_maps = ContactMapContainer()
        model        = structure_container.structure[0]
        start_pos = structure_container.startpos
        ...
        for chain_name in structure_container.chains:
            ...
            if chain['seqres-seq'] is not None and len(chain['seqres-seq']) > 0: #该链有残基
                ...
                #residues           = model[chain_name].get_residues()
                reindexed_residues = list(model[chain_name].get_residues())
                #print(reindexed_residues)
                #print(len(reindexed_residues))
                final_residue_list = []

                picked_residues = 0
                non_canonicals_or_het = 0

                for i in range(len(chain['seqres-seq'])):
                    final_residue_list.append(None)

                for i in range(len(reindexed_residues)):
                    candidate_residue = reindexed_residues[i]
                    picked_index = candidate_residue.id[1]
                    pos = picked_index-start_pos
                    if pos < len(chain['seqres-seq']):
                        #print(picked_index)
                        #print(pos)
                        final_residue_list[pos] = candidate_residue
                    else:
                        break
    ...
    def __calc_residue_dist(self, residue_one, residue_two):
        ...
        test = {residue_one, residue_two}
        test =  {residue_one, residue_two} & {None}
    ...
    def __diagnolize_to_fill_gaps(self, distance_matrix, length):
        # Create CMAP from __diagnolize_to_fill_gaps
